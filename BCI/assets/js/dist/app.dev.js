...
var active_link = string.substring(string.indexOf('BCI/') + 4, string.length - 5);
...
$(document).ready(function () {
  ...
  var sub_menu = $('.menu-link .sub_menu').append('<span class="close_sub"></span>');
  $('.menu-link .sub_menu').parent().append('<span class="open_sub"></span>');
});

var renderMenu = function renderMenu(active_link) {
...
var renderLine = function renderLine() {
  ...
  for (var i = 1; i < 21; i++) {
    line += '<div><svg xmlns="http://www.w3.org/2000/svg" viewBox="' + view_box + '">' + '<path d="' + line_path + '"></path>' + '</svg></div>';
  }
...
var getMenu = function getMenu(active_link) {
  ...
  $.each(list_menu, function (index, value) {
    ...
    if (value.hasOwnProperty('sub')) {
      var submenu = '<div class="sub_menu"><ul>';
...
var renderPost = function renderPost(template, step) {
  var _template = template.html();

  var link = template.attr('data-link');
